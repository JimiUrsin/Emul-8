...
std::pair<uint32_t*, bool> Display::write_to_vram(const uint8_t height, const uint8_t* pixel_data, const uint8_t& x_pos, const uint8_t& y_pos) {
...
bool Display::get_key(const uint8_t& key) {
    SDL_PumpEvents();
    const SDL_Scancode& scancode = key_map.at(key);
    return keyboard_state[scancode] == 1;
}

bool Display::draw_sprite(const uint8_t& height, const uint8_t* pixel_data, const uint8_t& x, const uint8_t& y) {
